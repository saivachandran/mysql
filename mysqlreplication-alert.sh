...
#echo $slaveRunning
   ...
   if  [ "$slaveRunning" != 1 -o $slaveSQLRunning != 1 -o  $secondsBehind != 1 ]; then  
   
   
     echo "$server_name mysql replication issue found" | mail -s "Mysql Replication alert" saiva.chandran@tendercuts.in < replicationStatus.txt
